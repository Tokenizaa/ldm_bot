...
export class CrawlerService {
  ...
  async login(): Promise<boolean> {
    ...
    if (!email || !password) {
      // Product catalog is publicly accessible; authentication is not required for crawling.
      return true;
    }
    ...
    try {
      ...
      console.warn('Login não necessário ou indisponível; continuando com catálogo público:', error instanceof Error ? error.message : String(error));
      return true;
    }
  }
  ...
  private async extractProductFromUrl(productUrl: string): Promise<AffiliateLink | null> {
    ...
    try {
      ...
      const data = await this.page.evaluate((): ScrapedProduct | null => {
        ...
        const schemaUrl = clean(productSchema?.url || offer?.url);
        ...
        const url = schemaUrl || location.href;
  ...
  async runCrawl(): Promise<{ success: boolean; products: AffiliateLink[]; error?: string }> {
    try {
      ...
      if (products.length === 0) {
        throw new Error(`Nenhum produto encontrado na categoria: ${category.url}`);
      }

      // Persist every valid scraped product. Scoring and publication selection happen later.
