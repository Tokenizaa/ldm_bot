...
type Candidate = { id: string; affiliate_url: string; brand: string | null; product_name: string | null; category: string | null; identityKey: string };
...
async function loadRecentAffiliateIds(periodStart: string): Promise<Set<string>> {
  ...
  const { data, error } = await supabase.from('publication_history').select('affiliate_link_id').lt('published_at', `${periodStart}T00:00:00.000Z`).order('published_at', { ascending: false }).limit(MONTHLY_PLAN_TOTAL_POSTS);
  if (error) throw error;
  return new Set((data ?? []).map((row) => row.affiliate_link_id));
}

async function loadCandidates(): Promise<Candidate[]> {
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase.from('affiliate_links').select('id,affiliate_url,brand,product_name,category,product_identity_key').eq('active', true);
  if (error) throw error;
  return (data ?? []).map((row) => ({ id: row.id, affiliate_url: row.affiliate_url, brand: row.brand, product_name: row.product_name, category: row.category, identityKey: row.product_identity_key || buildProductIdentityKey({ brand: row.brand, productName: row.product_name, category: row.category }) })).sort(rank);
}
...
export async function fillMonthlyPlan(planId: string) {
  ...
  const updates = posts.slice(0, selected.length).map((post, index) => {
    ...
    return { id: post.id, affiliate_link_id: candidate.id, link: candidate.affiliate_url, image_url: null, status: 'draft' };
...
export async function generateMonthlyPlanCopies(planId: string) {
  const supabase = getSupabaseAdmin();
  const { data: posts, error } = await supabase.from('posts').select('id,affiliate_link_id').eq('plan_id', planId).order('slot_index');
  if (error) throw error;
  if (!posts?.length) throw new Error('Plano sem posts para gerar copy');
  let generated = 0;
  for (const post of posts) {
    if (!post.affiliate_link_id) continue;
    const { data: link, error: linkError } = await supabase.from('affiliate_links').select('*').eq('id', post.affiliate_link_id).single();
    if (linkError) throw linkError;
    const content = await generateProductCopy(link);
    const { error: updateError } = await supabase.from('posts').update({ content, status: 'draft' }).eq('id', post.id);
