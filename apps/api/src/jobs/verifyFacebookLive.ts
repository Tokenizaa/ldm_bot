...
const main = async () => {
  ...
  try {
    ...
    const url = page.url();
    const loginRequired = /\/login|\/checkpoint|\/recover|two_step_verification/.test(url) || await page.locator('input[name="email"], input[id="email"]').isVisible({ timeout: 1500 }).catch(() => false);
    const result = {
