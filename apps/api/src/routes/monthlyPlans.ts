...
export async function monthlyPlanRoutes(fastify: FastifyInstance) {
  fastify.post('/monthly-plans', async (request, reply) => {
    ...
    try {
      ...
      return reply.status(500).send({ success: false, error: error instanceof Error ? error.message : 'Erro ao criar plano mensal' });
    }
  });
  ...
  fastify.post('/monthly-plans/:id/fill', async (request, reply) => {
    ...
    try {
      ...
      request.log.error(error);
      return reply.status(500).send({ success: false, error: error instanceof Error ? error.message : 'Erro ao selecionar produtos' });
    }
  });
  ...
  fastify.post('/monthly-plans/:id/generate', async (request, reply) => {
    ...
    if (!parsed.success) return reply.status(400).send({ success: false, error: 'ID inválido' });
    try {
      const result = await generateMonthlyPlanCopies(parsed.data.id);
      return { success: true, data: result };
    } catch (error) {
      request.log.error(error);
      return reply.status(500).send({ success: false, error: error instanceof Error ? error.message : 'Erro ao gerar copies' });
    }
  });
  ...
  fastify.get('/monthly-plans/:id', async (request, reply) => {
    ...
    try {
      ...
      request.log.error(error);
      return reply.status(500).send({ success: false, error: error instanceof Error ? error.message : 'Erro ao consultar plano mensal' });
    }
  });
}
